...
export default function App(){
  ...
  const status = useMemo(()=>{
    const s = { infoBasica:'inprogress', vehiculo:'pending', preventivoMG:'pending', sistemaTierras:'pending', infraestructuraTorre:'pending', inventarioEquipos:'pending', mantenimientoSitio:'pending', fotos:'pending', firma:'pending' }
    if (inspection.sitio?.nombre && inspection.sitio?.idSitio && inspection.sitio?.coords) s.infoBasica='done'
    if (Object.keys(inspection.vehiculo||{}).length) s.vehiculo='inprogress'
  ...
  return (
    <div className="min-h-screen">
      ...
      {route==='infoBasica' && <InfoBasica inspection={inspection} setField={setField} markStart={markStart} />}
      {route==='vehiculo' && <Vehiculo inspection={inspection} setField={setField} />}
      {route==='sistemaTierras' && <SistemaTierras inspection={inspection} save={save} setField={setField} />}
      {route==='infraestructuraTorre' && <InfraestructuraTorre inspection={inspection} save={save} setField={setField} />}
      {route==='inventarioEquipos' && <InventarioEquipos inspection={inspection} save={save} />}
      {route==='mantenimientoSitio' && <MantenimientoSitio inspection={inspection} save={save} />}
      {route==='firma' && (
        <div className="container px-4 py-6 space-y-4">
          ...
          <div className="flex gap-2 justify-end">
            ...
            <button className="btn btn-primary" onClick={()=>{ markEnd(); downloadPDF(inspection) }}>Finalizar e imprimir PDF</button>
          </div>
        </div>
      )}
    </div>
